    ...
    <Card no3d className="overflow-hidden rounded-2xl border-border/60 shadow-xs">
      ...
      <CardContent className="space-y-4 p-3.5 pt-1 sm:space-y-5 sm:p-5 sm:pt-2">
        ...
        <div className="space-y-3 rounded-2xl border border-border/40 bg-muted/20 p-3 sm:p-4">
          ...
          {preview.length > 0 && <div className="overflow-hidden rounded-xl border bg-background">{preview.map((item, index) => <div key={`${item.client_id}-${item.scheduled_at}`} className={`flex min-h-16 items-center gap-3 px-3 py-2.5 sm:px-4 ${index ? "border-t" : ""}`}><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{item.client_name}</p><p className="mt-0.5 text-xs text-muted-foreground">{item.contracts} {item.contracts === 1 ? "contrato" : "contratos"}</p></div><p className="shrink-0 text-sm font-bold tabular-nums sm:text-base">{Number(item.amount || 0).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</p></div>)}</div>}
