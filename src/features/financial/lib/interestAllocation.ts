...
/* ------------------------------------------------------------------ *
 * Juros Recebidos — competência do mês do PAGAMENTO (histórico travado)
 * ------------------------------------------------------------------ */

/**
 * Aloca juros considerando APENAS os pagamentos ocorridos até `cutoffDate`
 * (inclusive, formato YYYY-MM-DD).
 *
 * Isso torna o histórico imutável: pagamentos futuros (inclusive quitações e
 * juros pagos em atraso) não podem reprocessar a alocação de meses já
 * encerrados. Cada mês fechado é calculado como se "fechasse o caixa" no
...
export function allocateInterestByPaymentUpTo(
  ...
  const subset = cutoff ? payments.filter((p) => (p.date ?? "") <= cutoff) : payments;
...
/**
 * "Juros Recebidos" de um período, no regime oficial do app:
 *  - o juros pertence ao mês em que o PAGAMENTO foi efetivamente realizado
 *    (juros de maio pagos em julho entram em julho);
 *  - o total de um período encerrado é congelado: só depende dos pagamentos
 *    ocorridos até o fim do período.
 */
...
/**
 * Série mensal de "Juros Recebidos" (chaves YYYY-MM), cada mês travado com
 * corte no seu próprio fim — nunca reprocessado por pagamentos posteriores.
 */
export function monthlyInterestReceived(
  loans: AllocLoanLike[],
  payments: AllocPaymentLike[],
  monthKeys: string[],
): Map<string, number> {
  ...
  for (const key of monthKeys) {
    ...
    if (!Number.isFinite(y) || !Number.isFinite(m)) { out.set(key, 0); continue; }
    const start = new Date(y, m - 1, 1);
    const end = new Date(y, m, 0);
    out.set(key, sumInterestReceivedInPeriod(loans, payments, start, end));
  }
  return out;
}
