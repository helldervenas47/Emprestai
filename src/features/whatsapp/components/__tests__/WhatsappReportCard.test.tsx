...
describe("WhatsappReportCard — Envio de Relatórios e Resumo Operacional pelo WhatsApp", () => {
  ...
  it("formata corretamente a mensagem do Relatório de Cobranças com base na aba 'A cobrar'", () => {
    ...
    expect(message).toContain("• *João da Silva* (1 cont.): R$ 500,00 (J: R$ 50,00)");
    expect(message).toContain("• *Maria Santos* (1 cont.): R$ 330,00 (J: R$ 30,00)");
    expect(message).toContain("*Total enviado (2 cont.): R$ 830,00 (J: R$ 80,00)*");
    expect(message).toContain("⚠️ *COBRANÇAS NÃO ENVIADAS*");
    expect(message).toContain("• *Pedro Santos* (1 cont.): R$ 390,00 (J: R$ 40,00)");
    expect(message).toContain("*Total não enviado (1 cont.): R$ 390,00 (J: R$ 40,00)*");
  ...
  it("consolida múltiplos empréstimos do mesmo cliente em apenas uma linha com juros e valor total somados", () => {
    ...
    // Deve ter apenas UMA ocorrência de João da Silva com (2 cont.) e a soma dos valores
    expect(message).toContain("• *João da Silva* (2 cont.): R$ 500,00 (J: R$ 50,00)");
    const count = (message.match(/João da Silva/g) || []).length;
    expect(count).toBe(1);
    expect(message).toContain("Total de cobranças: *2*");
    expect(message).toContain("*Total enviado (2 cont.): R$ 500,00 (J: R$ 50,00)*");
  });
});
