      ...
      if (recurring) {
        try {
          ...
          if (contracts.data?.length === 1) {
            await admin.from("billing_contracts").upsert({ environment, subscription_id: contracts.data[0].id, order_id: order.id });
            foundPayment = (await asaasFetch(`/subscriptions/${encodeURIComponent(contracts.data[0].id)}/payments`)).data?.[0];
          }
