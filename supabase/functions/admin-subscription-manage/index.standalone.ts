// Standalone build (inline shared modules) para colar no Supabase Dashboard.
// Gerado a partir de: index.ts + _shared/require-admin.ts + _shared/external-supabase.ts
...
function getExternalSupabaseUrl(): string {
  ...
  return _required("EXTERNAL_SUPABASE_URL");
}
function getExternalServiceRoleKey(): string {
  ...
  return _required("EXTERNAL_SUPABASE_SERVICE_ROLE_KEY");
}
...
// ---------- index.ts (original, sem os imports) ----------
