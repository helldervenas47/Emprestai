...
Deno.serve(async (req) => {
  ...
  if (!isCron) {
    // Se não for cron secret, tenta validar token de usuário autenticado
    const token = req.headers.get("Authorization") || "";
    if (!token) return unauthorized(cors);
    const anon = Deno.env.get("EXTERNAL_SUPABASE_ANON_KEY")!;
    const userClient = createClient(Deno.env.get("EXTERNAL_SUPABASE_URL")!, anon, { global: { headers: { Authorization: token } } });
    const { data: auth } = await userClient.auth.getUser();
    if (!auth?.user) return unauthorized(cors);
  }
  ...
  try {
    ...
    if (!loans?.length || loans.some((loan: any) => loan.status === "paid")) throw new Error("Parcela já quitada ou indisponível");
    
    const { data: config } = await admin.from("whatsapp_billing_schedule").select("provider, base_url, instance_id, api_key").eq("owner_id", item.user_id).maybeSingle();
    const apiKey = (config as any)?.api_key || (config?.provider === "wppconnect"
    ...
    if (!config?.base_url || !config?.instance_id || !apiKey) throw new Error("WhatsApp não configurado (verifique URL, Instância e API Key em Configurações)");
