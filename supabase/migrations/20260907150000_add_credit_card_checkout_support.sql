-- Migration para suportar checkout direto via Cartão de Crédito

-- 1. Atualizar constraint da tabela billing_orders para aceitar 'credit_card'
ALTER TABLE public.billing_orders 
  DROP CONSTRAINT IF EXISTS billing_orders_checkout_kind_check;

ALTER TABLE public.billing_orders 
  ADD CONSTRAINT billing_orders_checkout_kind_check 
  CHECK (checkout_kind IN ('pix', 'recurring', 'credit_card'));

-- 2. Atualizar função RPC billing_prepare_order
...
BEGIN
  ...
  IF _kind NOT IN ('pix', 'recurring', 'credit_card') OR _cycle NOT IN ('monthly', 'semestral', 'annual') OR _cents <= 0 THEN
  ...
  IF FOUND THEN
    ...
    IF o.plan_id = _plan AND o.cycle = _cycle AND o.checkout_kind = _kind AND o.status IN ('pending', 'creating') THEN
      RETURN jsonb_build_object('created', false, 'order', to_jsonb(o));
    ELSE
      -- Se o usuário mudou de plano/ciclo ou método, cancela a ordem anterior e cria uma nova
